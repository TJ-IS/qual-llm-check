# AIS Basket 全文筛选：唯一客观指标提升 + 明确 Benchmark 表述
...
只有两个模块全部通过，`strict_include=true`。
  ...
  "evidence_pointers": [
    {
      ...
      "supports": "metric | core_goal | benchmark | exclusion"
    }
  ],
  "exclusion_trigger_codes": [],
  "decision_reason_cn": "覆盖客观指标、唯一核心目标、benchmark 三方面",
  "confidence": 0.0,
...
- `strict_include` 必须等于 `objective_metric.pass` 与 `benchmark.pass` 的逻辑与。
