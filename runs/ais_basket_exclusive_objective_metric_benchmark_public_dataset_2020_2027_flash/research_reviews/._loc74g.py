# -*- coding: utf-8 -*-
import io, os, glob
base = "E:/github/qual-llm-check-IS-utd/runs/ais_basket_exclusive_objective_metric_benchmark_public_dataset_2020_2027_flash/research_reviews"
fs = {}
for p in glob.glob(base + "/*_v3.1.md"):
    fn = os.path.basename(p)
    if fn.startswith("36_"): fs["36"] = p
t = io.open(fs["36"], encoding="utf-8").read()
lines = t.split("\n")
out = io.open(base + "/_loc74g.txt", "w", encoding="utf-8")
for pat in ["总结", "归纳", "关注", "呈现了该框架"]:
    start = 0
    out.write("PAT %s\n" % pat)
    while True:
        idx = t.find(pat, start)
        if idx < 0: break
        line_no = t.count("\n", 0, idx) + 1
        s = max(0, idx-40); e = min(len(t), idx+50)
        out.write("  line %d: ...%s...\n" % (line_no, t[s:e].replace("\n"," ")))
        start = idx + 1
out.close()
print("done")
