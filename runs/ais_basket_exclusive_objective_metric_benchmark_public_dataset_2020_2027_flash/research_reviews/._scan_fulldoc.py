...
def is_junk(s):
    if re.search(r'[a-zA-Z]', s) and ('et al' in s or '&' in s or 'Fang' in s or 'Yang' in s or 'Zhang' in s or 'Wang' in s or 'Lin' in s or 'Li' in s or 'AgentShield' in s or 'SWE' in s or 'MCP' in s or 'AUC' in s or 'IS' in s or 'T' in s or 'D' in s or 'RQ' in s):
        return True
...
for k, fn in files.items():
    ...
    for s in sorted(reps, key=lambda x: -len(x)):
        ...
        seen.add(s)
        out.append((len(s), s, reps[s]))
    for L, s, cnt in out[:40]:
        print(L, s, cnt)
