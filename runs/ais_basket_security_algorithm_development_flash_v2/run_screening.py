...
ALGORITHM_GATE_KEYS = (
    "novel_method_is_core_contribution",
    "method_implemented",
    "empirical_evaluation_present",
    "algorithm_not_mere_tool",
)
ALLOWED_ALGORITHM = {"novel_algorithm_with_evaluation"}
ALL_ALGORITHM = ALLOWED_ALGORITHM | {
    "method_adaptation_application_only", "conceptual_or_theoretical_only",
    "empirical_behavioral_only", "review_or_commentary",
    "no_algorithm", "unclear",
}
ALLOWED_PUBLICNESS = {"public", "private_or_nonpublic", "mixed", "unclear"}
...
def validate_decision(payload: dict[str, Any], record_id: str) -> dict[str, Any]:
    ...
    algorithm = require_dict(payload.get("algorithm_development"), "algorithm_development")
    publicness = require_dict(payload.get("data_publicness"), "data_publicness")
    security_gates = require_dict(security.get("gates"), "security_relevance.gates")
    algorithm_gates = require_dict(algorithm.get("gates"), "algorithm_development.gates")

    security_values = [require_bool(security_gates.get(key), f"security_relevance.gates.{key}") for key in SECURITY_GATE_KEYS]
    algorithm_values = [require_bool(algorithm_gates.get(key), f"algorithm_development.gates.{key}") for key in ALGORITHM_GATE_KEYS]

    security_status = str(security.get("status") or "")
    algorithm_status = str(algorithm.get("status") or "")
    publicness_status = str(publicness.get("status") or "")
    if security_status not in ALL_SECURITY:
        raise ValueError(f"invalid security status: {security_status!r}")
    if algorithm_status not in ALL_ALGORITHM:
        raise ValueError(f"invalid algorithm status: {algorithm_status!r}")
    if publicness_status not in ALLOWED_PUBLICNESS:
        raise ValueError(f"invalid data_publicness status: {publicness_status!r}")

    security_expected = all(security_values) and security_status in ALLOWED_SECURITY
    algorithm_expected = all(algorithm_values) and algorithm_status in ALLOWED_ALGORITHM
    security_pass = require_bool(security.get("pass"), "security_relevance.pass")
    algorithm_pass = require_bool(algorithm.get("pass"), "algorithm_development.pass")
    if security_pass != security_expected or algorithm_pass != algorithm_expected:
    ...
    if strict_include != (security_pass and algorithm_pass):
        raise ValueError("strict_include must equal both module passes")
...
def write_reports(
    ...
    summary = {
        ...
        "algorithm_pass_count": sum(
            bool((row.get("algorithm_development") or {}).get("pass")) for row in rows
        ),
        ...
        "algorithm_status_counts": dict(sorted(Counter(str((row.get("algorithm_development") or {}).get("status") or "") for row in rows).items())),
        "data_publicness_status_counts": dict(sorted(Counter(str((row.get("data_publicness") or {}).get("status") or "") for row in rows).items())),
        "usage": {
    ...
    fields = [
        ...
        "algorithm_pass",
        "algorithm_status",
        "algorithm_gates_json",
        "method_name_cn",
        "evaluation_cn",
        "algorithm_reason_cn",
        "data_publicness_status",
        "data_sources_cn",
    ...
    with tmp_csv.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        for row in rows:
            ...
            algorithm = row.get("algorithm_development") or {}
            publicness = row.get("data_publicness") or {}
            writer.writerow(
                {
                    ...
                    "algorithm_pass": algorithm.get("pass"),
                    "algorithm_status": algorithm.get("status"),
                    "algorithm_gates_json": json.dumps(algorithm.get("gates") or {}, ensure_ascii=False),
                    "method_name_cn": algorithm.get("method_name_cn"),
                    "evaluation_cn": algorithm.get("evaluation_cn"),
                    "algorithm_reason_cn": algorithm.get("reason_cn"),
                    "data_publicness_status": publicness.get("status"),
                    "data_sources_cn": publicness.get("data_sources_cn"),
        ...
        for row in sorted(
            ...
            markdown.extend(
                [
                    ...
                    f"- Algorithm status: {algorithm.get('status', '')}",
                    f"- Method: {algorithm.get('method_name_cn', '')}",
                    f"- Evaluation: {algorithm.get('evaluation_cn', '')}",
                    f"- Algorithm reason: {algorithm.get('reason_cn', '')}",
                    f"- Data publicness: {publicness.get('status', '')} / {publicness.get('data_sources_cn', '')}",
    ...
    write_markdown("strict_matches.md", "Security-relevant algorithm development (AIS Basket)", strict_matches)
