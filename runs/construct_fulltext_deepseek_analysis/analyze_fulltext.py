#!/usr/bin/env python3
"""Classify Otero full texts with DeepSeek for construct measurement and OR integration."""
...
def normalize_final(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    non_questionnaire = payload.get("non_questionnaire_construct_measurement")
    operations_research = payload.get("operations_research_with_construct")
    if not isinstance(non_questionnaire, bool):
        raise ValueError("non_questionnaire_construct_measurement must be boolean")
    if not isinstance(operations_research, bool):
        raise ValueError("operations_research_with_construct must be boolean")
    explicit_constructs = normalized_list(payload.get("explicit_constructs"))
    measured_constructs = normalized_list(payload.get("measured_constructs"))
    or_methods = normalized_list(payload.get("operations_research_methods"))
    or_evidence = normalized_list(payload.get("operations_research_evidence"))
    try:
    ...
    return {
        ...
        "construct_relevance_summary_cn": str(payload.get("construct_relevance_summary_cn") or "").strip(),
        "explicit_constructs": explicit_constructs,
        "non_questionnaire_construct_measurement": non_questionnaire,
        "non_questionnaire_measurement_summary_cn": str(
            payload.get("non_questionnaire_measurement_summary_cn") or ""
        ).strip(),
        "measured_constructs": measured_constructs,
        "operations_research_with_construct": operations_research,
        "operations_research_summary_cn": str(payload.get("operations_research_summary_cn") or "").strip(),
        "operations_research_methods": or_methods,
        "operations_research_evidence": or_evidence,
...
def write_reports(
    ...
    fields = [
        ...
        "explicit_constructs",
        "non_questionnaire_construct_measurement",
        "non_questionnaire_measurement_summary_cn",
        "operations_research_with_construct",
        "operations_research_summary_cn",
        "operations_research_methods",
        "confidence",
        "analysis_mode",
        "fulltext_chars",
    ]
    with temp_csv.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        for source_file in sorted(decisions):
            ...
            row["explicit_constructs"] = json.dumps(
                row.get("explicit_constructs", []), ensure_ascii=False
            )
            row["operations_research_methods"] = "; ".join(
                str(value) for value in row.get("operations_research_methods", [])
            )
    ...
    summary = {
        ...
        "non_questionnaire_construct_measurement_true": sum(
            row.get("non_questionnaire_construct_measurement") is True for row in values
        ),
        "operations_research_with_construct_true": sum(
            row.get("operations_research_with_construct") is True for row in values
        ),
        "usage": {
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_concurrency) as pool:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                completed_now += 1
                flag_a = record["non_questionnaire_construct_measurement"]
                flag_b = record["operations_research_with_construct"]
                print(
                    f"[{completed_now + failed_now}/{len(pending)}] OK {path.name} "
                    f"non_questionnaire={flag_a} or_with_construct={flag_b}"
                )
            except Exception as exc:
                failed_now += 1
                append_jsonl(
